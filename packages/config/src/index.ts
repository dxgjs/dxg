...
function validateDXGConfig(config: unknown): DXGConfig {
  if (typeof config !== 'object' || config === null) {
    logger.warn('Configuration must be an object. Using default configuration.');
    return {};
  }
  ...
  if ('name' in config && config.name !== undefined) {
    if (typeof config.name === 'string') {
      ...
      logger.warn(`Configuration field 'name' must be a string. Received: ${typeof config.name}. Ignoring this field.`);
    }
  }
  ...
  if ('version' in config && config.version !== undefined) {
    if (typeof config.version === 'string') {
      ...
      logger.warn(`Configuration field 'version' must be a string. Received: ${typeof config.version}. Ignoring this field.`);
    }
  }
  ...
  if ('description' in config && config.description !== undefined) {
    if (typeof config.description === 'string') {
      ...
      logger.warn(`Configuration field 'description' must be a string. Received: ${typeof config.description}. Ignoring this field.`);
    }
  }
  ...
  if (unknownFields.length > 0) {
    logger.warn(`Unknown configuration fields: ${unknownFields.join(', ')}. These fields will be ignored.`);
  }
...
export async function loadConfig(rootPath: string): Promise<DXGConfig> {
  ...
  try {
    ...
    try {
      ...
      logger.warn(`Failed to parse dxg.config.json as JSON: ${err instanceof Error ? err.message : String(err)}. Using default configuration.`);
  ...
  try {
    ...
    if (typeof parsed !== 'object' || parsed === null) {
      logger.warn(`dxg.config.js did not export an object. Using default configuration.`);
      return defaultConfig;
    }
    ...
    if (err instanceof Error && 'code' in err && typeof err.code === 'string' && err.code !== 'MODULE_NOT_FOUND') {
      logger.warn(`Failed to load dxg.config.js: ${err.message}. Using default configuration.`);
    }
